...
const BlackListEdit = () => {
  ...
  const settingPage = async () => {
    ...
    if (user?.level >= 40) {
      ...
      data.mcht_id = user?.id;
    }


    if (router.query?.edit_category == 'edit') {
      data = await apiManager('black-lists', 'get', {
        id: router.query.id
      })
    }
    setItem(data);
  ...
  const onSave = async () => {
    ...
    if (data?.id) {//수정
      result = await apiManager('black-lists', 'update', data);
    } else {//추가
      result = await apiManager('black-lists', 'create', data);
    }
    if (result) {
...
BlackListEdit.getLayout = (page) => <ManagerLayout>{page}</ManagerLayout>;
