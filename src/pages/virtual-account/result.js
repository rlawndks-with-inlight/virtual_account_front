...
const VirtualAccountResult = () => {
    ...
    const onCheckPhoneNumCheck = async () => {
        let result = undefined;
        result = await apiManager('virtual-accounts/daily-auth-check', 'create', authItem);
        if (result) {
                ...
                <Col style={{ marginTop: '2rem' }}>
                    ...
                    {currentTab == 1 &&
                        <>
                            <Card sx={{ p: 2, maxWidth: '500px', margin: 'auto', width: '90%' }}>
                                <Col style={{ rowGap: '0.5rem' }}>
                                    {(item?.last_auth_date.substring(0, 10) == returnMoment().substring(0, 10) || themeDnsData?.deposit_corp_type != 7) ?
                                        <>
